...
func (m *manager) isCloudEventAMatch(ce *event.Event, natsSubject string,
	...
	isSubjectMatch := criteria.Subject == "" ||
		(criteria.Subject != "" && criteria.Subject == natsSubject)
	isCESourceMatch := criteria.CloudEventSource == "" ||
		(criteria.CloudEventSource != "" && criteria.CloudEventSource == ce.Source())
	isCESubjectMatch := criteria.CloudEventSubject == "" ||
		(criteria.CloudEventSubject != "" && criteria.CloudEventSubject == ce.Subject())
	isCEEventMatch := criteria.CloudEventType == "" ||
		(criteria.CloudEventType != "" && criteria.CloudEventType == ce.Type())

	if isSubjectMatch && isCESourceMatch && isCESubjectMatch && isCEEventMatch {
		return true
	}
