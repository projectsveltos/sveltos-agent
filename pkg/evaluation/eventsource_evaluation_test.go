...
var _ = Describe("Manager: healthcheck evaluation", func() {
	...
	It("isMatchForEventSource: evaluates deployment as match", func() {
		...
		var isMatch bool
		isMatch, err = evaluation.IsMatchForEventSource(manager, depl, degradedDeploymentLuaScript, klogr.New())
		Expect(err).To(BeNil())
	...
	It("isMatchForEventSource: evaluates deployment as non match", func() {
		...
		replicas := 3
		depl, err := libsveltosutils.GetUnstructured([]byte(fmt.Sprintf(deploymentTemplate, replicas, replicas, replicas, replicas)))
		Expect(err).To(BeNil())
	...
	It("fetchEventSourceResources: fetches all resources mathing label selectors", func() {
		...
		manager := evaluation.GetManager()
		Expect(manager).ToNot(BeNil())
		deployments, err := evaluation.FetchEventSourceResources(manager, context.TODO(), eventSource)
		Expect(err).To(BeNil())
	...
	It("getEventMatchingResources: returns resources matching an EventSource", func() {
		...
		Expect(waitForObject(context.TODO(), testEnv.Client, healthyNonMatchingDepl)).To(Succeed())
	...
	It("cleanEventReport marks eventReport as deleted", func() {
		...
		err := c.Get(context.TODO(), types.NamespacedName{Name: eventReport.Name, Namespace: utils.ReportNamespace},
			eventReport)
		Expect(err).To(BeNil())
