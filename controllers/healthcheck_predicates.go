...
// ConfigMapPredicates predicates for ConfigMaps. HealthCheckReconciler watches ConfigMap events
...
func ConfigMapPredicates(logger logr.Logger) predicate.Funcs {
	return predicate.Funcs{
		UpdateFunc: func(e event.UpdateEvent) bool {
			...
			if oldConfigMap == nil {
				log.V(logs.LogVerbose).Info("Old ConfigMap is nil. Reconcile HealthChecks.")
				return true
			}

			if !reflect.DeepEqual(oldConfigMap.Data, newConfigMap.Data) {
				log.V(logs.LogVerbose).Info(
					"ConfigMap Data changed. Will attempt to reconcile associated HealthChecks.",
				)
				return true
			}
			...
			log.V(logs.LogVerbose).Info(
				"ConfigMap did not match expected conditions.  Will not attempt to reconcile associated HealthChecks.")
			return false
		},
...
// SecretPredicates predicates for Secrets. HealthCheckReconciler watches Secret events
// and react to those by reconciling itself based on following predicates
func SecretPredicates(logger logr.Logger) predicate.Funcs {
	return predicate.Funcs{
		UpdateFunc: func(e event.UpdateEvent) bool {
			...
			if oldSecret == nil {
				log.V(logs.LogVerbose).Info("Old Secret is nil. Reconcile HealthChecks.")
				return true
			}

			if !reflect.DeepEqual(oldSecret.Data, newSecret.Data) {
				log.V(logs.LogVerbose).Info(
					"Secret Data changed. Will attempt to reconcile associated HealthChecks.",
				)
				return true
			}
			...
			// otherwise, return false
			log.V(logs.LogVerbose).Info(
				"Secret did not match expected conditions.  Will not attempt to reconcile associated HealthChecks.")
			return false
		},
		CreateFunc: func(e event.CreateEvent) bool {
			return CreateFuncTrue(e, logger)
...
var (
	CreateFuncTrue = func(e event.CreateEvent, logger logr.Logger) bool {
		...
		log.V(logs.LogVerbose).Info(fmt.Sprintf(
			"%s did match expected conditions.  Will attempt to reconcile associated HealthChecks.",
	...
	GenericFuncFalse = func(e event.GenericEvent, logger logr.Logger) bool {
		...
		log.V(logs.LogVerbose).Info(fmt.Sprintf(
			"%s did not match expected conditions.  Will not attempt to reconcile associated HealthChecks.",
			e.Object.GetObjectKind()))
		return false
	}
)
