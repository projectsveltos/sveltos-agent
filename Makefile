...
set-version:
	...
	sed -i'' -e 's@--version=.*@--version='$(VERSION)'"@'  ./config/default/manager_auth_proxy_patch.yaml
	sed -i'' -e 's@- --version=.*@- --version='"$(VERSION)"'@'  ./manifest/mgmt_cluster_manifest.yaml
